...
```{.k .uiuck .rvk}
    syntax Int ::= sgn ( Int ) [function]
                 | abs ( Int ) [function]
 // -------------------------------------
    rule sgn(I) => -1 requires I >=Int (2 ^Int 255)
    rule sgn(I) => 1  requires I <Int (2 ^Int 255)
...
```{.k .uiuck .rvk}
 ...
 // -----------------------------------------
    ...
    rule W0 -Word W1 => chop( (W0 +Int (2 ^Int 256)) -Int W1 )
    rule W0 *Word W1 => chop( W0 *Int W1 )
    rule W0 /Word 0  => 0
...
```{.k .uiuck .rvk}
    syntax Int ::= Int "^Word" Int [function]
 // -----------------------------------------
    rule W0 ^Word W1 => (W0 ^Word (W1 /Int 2)) ^Word 2  requires W1 >=Int (2 ^Int 16) andBool W1 %Int 2 ==Int 0
    rule W0 ^Word W1 => (W0 ^Word (W1 -Int 1)) *Word W0 requires W1 >=Int (2 ^Int 16) andBool W1 %Int 2 ==Int 1
    rule W0 ^Word W1 => chop( W0 ^Int W1 )              requires W1 <Int (2 ^Int 16)
```
...
```{.k .uiuck .rvk}
 ...
 // -------------------------------------------
    rule ~Word W       => chop( W xorInt ((2 ^Int 256) -Int 1) )
    rule W0 |Word   W1 => chop( W0 |Int W1 )
    rule W0 &Word   W1 => chop( W0 &Int W1 )
...
```{.k .uiuck .rvk}
 ...
 // --------------------------------------------
    ...
    rule byte(N, W) => (W >>Int (256 -Int (8 *Int (N +Int 1)))) %Int (2 ^Int 8) requires N >=Int 0 andBool N <Int 32
```
